...
function testFindNeighbors3D
    % --- resolve src/ relative to this file so the test works from any cwd ---
    thisFile = mfilename('fullpath');        % .../GranE/tests/testPack/testFindNeighbors3D.m
    [thisDir, ~, ~] = fileparts(thisFile);   % .../GranE/tests/testPack

    testsDir = fileparts(thisDir);           % .../GranE/tests
    repoDir  = fileparts(testsDir);          % .../GranE
    srcDir   = fullfile(repoDir, 'src');     % .../GranE/src
